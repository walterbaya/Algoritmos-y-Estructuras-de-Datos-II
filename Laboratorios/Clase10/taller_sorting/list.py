...
class List(list):
    ...
    def crear_temporal(self, size=None):
        ...
        self.history.extend(['cmp' for k in range(len(self))])
    ...
    def __getslice__(self, i, j):
        if j == sys.maxsize: j=len(self)
        self.counter.add(j-i)
        self.history.extend(['cmp' for k in range(j-i)])
        return List(super(List, self).__getitem__(slice(i, j)), counter=self.counter, history=SliceHistory(i, self.history))
